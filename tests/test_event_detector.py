...
class EventDetectorTestCase(unittest.TestCase):
    ...
    def test_non_event_page_fails_when_only_one_option(self) -> None:
        detector = EventDetector()
        with tempfile.TemporaryDirectory() as temp_dir:
        ...
        self.assertFalse(result.is_event_page)
        self.assertEqual(result.option_count, 1)

    def _build_mock_event_page(self, output_path: Path, option_count: int) -> None:
